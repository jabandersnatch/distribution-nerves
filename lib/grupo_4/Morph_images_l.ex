defmodule Morphing do
  ...
  defp process_pixel(morphed_im, src_image, dest_image, interpolated_p, interpolated_q, {w, h}, frame, num_morphed_frames) do
    ...
    Enum.map(0..(div(length(interpolated_p), 2)), fn line ->
      ...
      u = ((w - x_p) * (x_q - x_p) + (h - y_p) * (y_q - y_p)) / ((x_q - x_p) * (x_q - x_p) + (y_q - y_p) * (y_q - y_p))

      v = ((w - x_p) * (y_p - y_q) + (h - y_p) * (x_q - x_p)) / :math.pow((x_q - x_p) * (x_q - x_p) + (y_q - y_p) * (y_q - y_p), 0.5)

      x_prime1 = [p1[0] + u * (q1[0] - p1[0]) + v * (p1[1] - q1[1]) / ((q1[0] - q1[0]) * (q1[0] - q1[0]) + (q1[1] - q1[1]) * (q1[1] - q1[1])), h + u * (q1[1] - p1[1]) + v * (q1[0] - p1[0]) / ((q1[0] - q1[0]) * (q1[0] - q1[0]) + (q1[1] - q1[1]) * (q1[1] - q1[1]))]

      x_prime2 = [p2[0] + u * (q2[0] - p2[0]) + v * (p2[1] - q2[1]) / ((q2[0] - q2[0]) * (q2[0] - q2[0]) + (q2[1] - q2[1]) * (q2[1] - q2[1])), h + u * (q2[1] - p2[1]) + v * (q2[0] - p2[0]) / ((q2[0] - q2[0]) * (q2[0] - q2[0]) + (q2[1] - q2[1]) * (q2[1] - q2[1]))]

      displacement1 = [x_prime1[0] - w, x_prime1[1] - h]

      displacement2 = [x_prime2[0] - w, x_prime2[1] - h]

      shortest_dist =
      if u >= 1 do
        :math.pow((x_q - w) * (x_q - w) + (y_q - h) * (y_q - h), 0.5)
      else
        if u <= 0 do
          :math.pow((x_p - w) * (x_p - w) + (y_p - h) * (y_p - h), 0.5)
        else
      ...
      dsum1 = [dsum1[0] + (line_weight * displacement1[0]), dsum1[1] + (line_weight * displacement1[1])]

      dsum2 = [dsum2[0] + (line_weight * displacement2[0]), dsum2[1] + (line_weight * displacement2[1])]

      weightsum = weightsum + line_weight

      end)

      x_prime1 = [w + (dsum1[0] / weightsum), h + (dsum1[1] / weightsum)]

      x_prime2 = [w + (dsum2[0] / weightsum), h + (dsum2[1] / weightsum)]
