defmodule ContadorPalabras.Coordinador do
  ...
  def start_link(opts \\ []) do
    GenServer.start_link(__MODULE__, %{contador: %{}, pendientes: 0, total: 0}, opts)
  end

  # Llamada pública para iniciar el conteo desde un archivo
  ...
  # Agrupa todas las cláusulas de handle_cast
  @impl true
  ...
  def handle_cast({:resultado, resultado}, state) do
    ...
    if updated_pendientes == 0 do
      IO.inspect(updated_contador, label: "Inicio de unificación de la información")
      IO.inspect(updated_contador, label: "Conteo definitivo de todo el texto")
      {:noreply, %{state | contador: %{}, pendientes: state.total, total: state.total}}
    else
      {:noreply, %{state | contador: updated_contador, pendientes: updated_pendientes}}
    end
  end
  ...
  defp dividir_texto(texto, num_nodos) do
    palabras = texto |> String.split(~r/\s+/, trim: true)
